...
## 🧪 假说化改造（2026-09-04，详见 REFACTOR_HYPOTHESIS_README.md）

每笔交易必须是可证伪的：**"因为 X，所以在 Y 买入；如果 Z 出现，说明我错了，离场；如果 W 出现，兑现离场。"** X/Y/Z/W 缺一即拒绝（不进调度不推送，signal_rejections 表留痕）。

| 改造 | 一句话 |
|---|---|
| 【一】假说出厂检查 | 沃尔德式"买93.88/损93.94"倒挂在生成阶段直接拒绝（原系统照发） |
| 【二】数据层一致性 | 量比<1而量能>10倍、换手<10%而量>10亿股 → 脏数据拦截（拦下 9/3 全部误触发与 9/4 沃尔德） |
| 【三】状态→事件 | 突破当日诞生（收阳+量能双腿）、N 日回踩有效、跌回突破位/退潮立即撤单、超期作废——不再连发两天 |
| 【四】买卖配对 | 四策略各自 Z/W；Z=X 的直接否定、宽度≥1.5×ATR；旧漂移止损+投票门降为辅助观察 |
| 【六】记录闭环 | 四行日志（假说原文/出入场/ZW归类/归因）+ 30 笔分层统计 + 滚动 50 笔期望为负自动下线 |
| 受众分流 | 买入事件只对空仓者成立；持仓者永远收到 持有/加仓/减仓/止损 四选一 |

新增表：signal_events / signal_rejections / strategy_status；trade_logs 新增 16 列（含假说四要素）。
回执 CLI 扩展：`--outcome logic_right|luck|logic_wrong`、`--stats`（分层统计）、`--strategies`（在线状态）。
