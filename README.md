...
并做质量过滤（碎单跳过 / 单次最多建议 4 只 / 单次建议总额上限 100 万）。
...
```
  ...
  做一件事:环境评估(自适应模式)→ 全量自选池信号 → 调度器按入场类型优先级排序、套预算和并发闸门
  (信号服务模式,单次最多建议 4 只/合计 ≤ 100 万)→ 主推送一条(环境+买卖信号),
  观察列表单独一条推送(2026-08-31 拆分,主报告内容超 PushPlus 2 万字上限会 999 失败)。
  pre_market 已合并进来,不用单独跑。
