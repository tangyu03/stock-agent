...
Phase2-C 数据源技术性缺陷整改（用户实测批评）：
  1. 两融：SSE/SZSE 官方接口失败时东财 datacenter 兜底；按日期记录
     接口拉取成败，区分「非两融标的」与「接口失败」
     （金海通 603061 沪市两融标的被报“非两融标的或深市接口失败”的铁证修复）
  2. 股东户数：报告期级滞后数据，统计截止日距今 >90 天 → 不参与投票
     （“股东户数增加110%”是过去报告期的变化，不是当下筹码信号）
  3. 主力资金流：大单拆单算法估算，机构拆单即可规避 → 票权 0.5；
     股东户数同降 0.5；两融/龙虎榜为交易所披露口径保持 1.0
...
# 【C-两融】按日期记录各市场接口拉取成败，用于区分“非两融标的”与“接口失败”
# （金海通 603061 沪市两融标的被报“非两融标的或深市接口失败”的直接根因：
#   沪市表拉取失败/当日缺行时，旧逻辑无法区分标的缺失与管道故障）
_margin_fetch_status: Dict[str, Dict[str, bool]] = {}  # {date: {"sse": bool, "szse": bool}}

# 【C-噪音降权】投票权重：主力（拆单算法噪音）/股东（报告期滞后）降权 0.5
_VOTE_WEIGHTS: Dict[str, float] = {
    "north_bound": 1.0,   # 两融：交易所披露
    "lhb": 1.0,           # 龙虎榜：交易所披露
    "main_force": 0.5,    # 主力净流入：大单拆单估算，噪音大
...
def _fetch_margin_balance_em(code: str, lookback_days: int = 12) -> Optional[Dict[str, float]]:
    """
    【C-两融】东财 datacenter 个股两融明细兜底。

    SSE/SZSE 官方接口失败/当日缺行时，用东财 RPTA_WEB_RZRQ_GGMX 拉单只
    个股的融资余额序列（近 N 日），返回 {YYYYMMDD: 融资余额(元)}。
    失败返回 None（测试可 monkeypatch 本函数）。
    """
    try:
        import requests
        ...
        params = {
            ...
            "sortColumns": "DATE",
            "sortTypes": "-1",
            "pageSize": str(int(lookback_days)),
            "pageNumber": "1",
        }
        ...
        out: Dict[str, float] = {}
        for row in rows:
            d = str(row.get("DATE") or "")[:10].replace("-", "")
            bal = _coerce_fund_flow_number(row.get("RZYE"))
            if d and bal and bal > 0:
                out[d] = float(bal)
        return out or None
    except Exception as e:
        logger.debug("两融东财兜底失败 %s: %s", code, str(e)[:60])
        return None
...
def _fetch_margin_balance(code: str) -> Dict[str, Any]:
    ...
    数据源：
      ...
      - 兜底：东财 datacenter RPTA_WEB_RZRQ_GGMX（单股序列）
    ...
    Phase2-C 改进（金海通铁证：沪市两融标的被报“非两融标的或深市接口失败”）：
      - 按日期记录 SSE/SZSE 接口拉取成败（_margin_fetch_status）
      - 官方表拿不到该股时，东财 datacenter 兜底再试一次
      - 官方表拉取成功且无此券 → 「非两融标的」（信息性）
      - 官方表拉取失败且兜底也无数据 → 「两融接口失败」（数据源故障，
        不再误导性暗示非两融标的）
    """
    ...
    try:
            ...
            try:
                ...
                if df_sse is not None and not df_sse.empty:
                    ...
                    if bal_col:
                        ...
                        _margin_fetch_status[fetch_date]["sse"] = True
            except Exception:
                pass
            # 深市（不稳定，失败不影响沪市数据）
            try:
                ...
                if df_szse is not None and not df_szse.empty:
                    ...
                    if bal_col:
                        for _, row in df_szse.iterrows():
                            ...
                            if c and bal > 0:
                                _margin_market_cache[fetch_date][c] = bal
                        _margin_fetch_status[fetch_date]["szse"] = True
            except Exception:
                pass  # 深市失败静默降级
        ...
        # 【C-两融】官方接口拿不到该股时，东财 datacenter 兜底再试一次
        em_used = False
        if latest_bal <= 0 or prev_bal <= 0:
            em_series = _fetch_margin_balance_em(code)
            if em_series:
                em_used = True
                for d, bal in em_series.items():
                    _margin_market_cache.setdefault(d, {})[code] = bal
                latest_bal = _margin_market_cache.get(latest_date, {}).get(code, 0)
                prev_bal = _margin_market_cache.get(prev_date, {}).get(code, 0)
        ...
        if latest_bal <= 0:
            ...
            if market == "bj":
                return {"vote": 0, "detail": "北交所无两融数据", "raw": {}}
            market_key = "sse" if market == "sh" else "szse"
            official_ok = _margin_fetch_status.get(latest_date, {}).get(market_key, False)
            if official_ok and not em_used:
                return {
                    "vote": 0,
                    "detail": "非两融标的（当日融资融券明细无此券）",
                    "raw": {"margin_target": False},
                }
            return {
                "vote": 0,
...
def _fetch_shareholder_count(code: str) -> Dict[str, Any]:
    ...
    try:
        ...
        # 【C-股东户数】报告期级数据：解析统计截止日，计算时效
        as_of_str = ""
        as_of = None
        try:
            raw_date = str(df.iloc[0][date_col])[:10].replace("-", "").replace("/", "")
        ...
        # 【C-股东户数】时效守卫：统计截止日距今 >90 天 → 报告期级滞后数据，
        # 不参与投票（“股东户数增加110%”是过去报告期的变化，不是当下筹码信号）
        _raw_common = {
            "latest": latest_count, "prev": prev_count, "change_pct": change_pct,
            "as_of": as_of_str, "age_days": age_days,
        }
        if age_days is not None and age_days > _SHAREHOLDER_STALE_DAYS:
            return {
                "vote": 0,
                "detail": (
                    f"股东户数{as_of_str}（报告期级，滞后{age_days}天>"
                    f"{_SHAREHOLDER_STALE_DAYS}天）不参与投票"
                ),
                "raw": {**_raw_common, "stale": True},
            }
        ...
        if change_pct < -0.02:  # 户数减少 >2%（筹码集中）
            return {
                "vote": 1,
                "detail": f"报告期{as_of_str or 'N/A'}: 股东户数减少 {change_pct*100:.1f}%（{prev_count:.0f}→{latest_count:.0f}，筹码集中）",
                "raw": dict(_raw_common),
            }
        elif change_pct > 0.02:  # 户数增加 >2%（筹码分散）
            return {
                "vote": -1,
                "detail": f"报告期{as_of_str or 'N/A'}: 股东户数增加 {change_pct*100:.1f}%（{prev_count:.0f}→{latest_count:.0f}，筹码分散）",
                "raw": dict(_raw_common),
            }
        else:
            return {
                "vote": 0,
                "detail": f"报告期{as_of_str or 'N/A'}: 股东户数持平（变化 {change_pct*100:.1f}%）",
                "raw": dict(_raw_common),
            }
    except Exception as e:
        err_msg = str(e)[:80]
